...
const TrainingPanel = () => {
  ...
  const loadModels = async () => {
    try {
      ...
      // Store full model objects with ELO and metadata
  ...
  const activateModel = async (modelName) => {
    try {
      ...
      toast.success(`🟢 Model ${modelName} is now active!`);
  ...
  const saveModel = async () => {
    try {
      ...
      toast.success(`✅ Model Saved: ${name}`);
      await loadModels();
      await loadActiveModel();
    } catch (error) {
      console.error('Error saving model:', error);
      toast.error('Failed to save model: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  const loadModel = async (name) => {
    try {
      ...
      toast.success(`✅ Model Loaded: ${name}`);
      await loadModels();
      await loadActiveModel();
    } catch (error) {
      console.error('Error loading model:', error);
      toast.error('Failed to load model: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  const exportModel = async (name) => {
    try {
      ...
      link.parentNode.removeChild(link);
      window.URL.revokeObjectURL(url);
      toast.success(`Model ${name} exported successfully!`);
    } catch (error) {
      console.error('Error exporting model:', error);
      toast.error('Failed to export model: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  const importModel = async (event) => {
    ...
    try {
      ...
      toast.success(`✅ Model ${name} imported and loaded successfully!`);
      await loadModels();
      await loadActiveModel();
    } catch (error) {
      console.error('Error importing model:', error);
      toast.error('Failed to import model: ' + (error.response?.data?.detail || error.message));
    }
